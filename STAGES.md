...
2. `NonZenoPass`
   ...
   * Анализирует каждый instant‑SCC и требует хотя бы одну реакцию с объявленным ранком.

   * Реакция объявляется напрямую через декоратор `@reaction(rank="remaining_steps", max_microsteps=32)`.

   * Если ранга нет — `ZEN001` с перечислением SCC и подсказкой «annotate rank or insert delay».

3. Runtime‑guard

   * Берёт минимальный `max_microsteps` среди реакций SCC (по умолчанию 20).
   * При превышении → `ZenoRuntimeError` + вывод состава SCC (см. `tests/test_stage7_zeno.py`).

### Definition of Done (Этап 7)

* Scheduler работает с `(t, micro)` и прерывает микро‑итерации по рангу/лимиту.
* В strict‑режиме instant‑SCC:
  * либо имеет `rank` и проходит `NonZenoPass`,
  * либо компиляция завершается `ZEN001`.
* Runtime‑guard детерминированно ловит остаточные Zeno‑циклы.

> С этого момента временная модель (`Tag=(t, micro)`) и подход к Zeno считаются **каноническими** и покрыты регрессией.
